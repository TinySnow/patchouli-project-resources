...
【依赖】
  ...
  字体：支持“字体文件路径优先 + 字体家族名回退”两种方式。
    - 推荐 macOS 直接使用系统字体文件：
        FONT_FILE = "/System/Library/Fonts/PingFang.ttc"
    - 或使用家族名：
        FONT_NAME = "PingFang SC"
  脚本启动时会打印最终命中的字体与字体文件路径。

【用法】
  将本脚本与 .xmind 文件放在同一目录，然后运行：

    python xmind_renderer_v5.py                  # 渲染全部画布
    python xmind_renderer_v5.py -s 哲学           # 只渲染"哲学"画布
    python xmind_renderer_v5.py -p 哲学/总论      # 只渲染该路径前缀下的节点
    python xmind_renderer_v5.py -s 哲学 -p 总论    # 在指定画布内渲染该局部
    python xmind_renderer_v5.py -i 其他文件.xmind # 指定其他 XMind 文件
    python xmind_renderer_v5.py -o 自定义输出目录  # 指定输出目录
    python xmind_renderer_v5.py -h                # 查看帮助
...
【增量更新推荐（只改一个分支时）】
  ...
  避免重跑整张图，显著缩短渲染时间。
    ...
    # 仅更新“哲学”画布下“总论”分支
    python xmind_renderer_v5.py -i ./学海计划.xmind -o ./maps -s 哲学 -p 总论

    # 仅更新更深层的小分支（完整路径）
    python xmind_renderer_v5.py -i ./学海计划.xmind -o ./maps -p 哲学/总论/哲学的分支体系
...
【输出目录结构】
  ...
  │           └── 叶子节点C.png
  └── ...
  若“画布名 == 根节点名”，会自动去重，避免出现“法学/法学/...”。

【文件命名规则】
  文件名仅使用当前节点标题（例如：伦理学.png），不拼接父路径。
  同时会先做标题清洗：
    - 删除（...）与 (...) 括号段（非贪婪）
    - 删除最后一个冒号（：或 :）及其后内容
    - 清理多余空白
...
def resolve_font():
    ...
    解析可用字体，优先级：
      ...
      2) FONT_NAME
      3) FONT_FALLBACKS 列表
      4) matplotlib 默认字体（最后兜底）
    返回：(FontProperties, 选中字体标签, 字体文件路径)

    说明：
      - 该函数只负责“选字体”，不处理字号/字重
      - 启动时打印返回值，便于定位“字体找不到”问题
    """
    # 1) 优先使用显式字体文件路径
    if (FONT_FILE or "").strip():
        p = os.path.expanduser(FONT_FILE.strip())
        if os.path.isfile(p):
            return font_manager.FontProperties(fname=p), f"file:{p}", p
        print(f"   ⚠️ FONT_FILE 不存在：{p}，将改用字体名称匹配。")

    # 2/3) 按字体家族名依次尝试
    candidates = []
    for fam in [FONT_NAME, *FONT_FALLBACKS]:
        fam = (fam or "").strip()
    ...
    for fam in candidates:
        try:
            ...
            return font_manager.FontProperties(fname=path), fam, path
        except Exception:
            continue

    # 4) 最后兜底：matplotlib 默认字体
    path = font_manager.findfont(font_manager.FontProperties(), fallback_to_default=True)
    return font_manager.FontProperties(fname=path), "matplotlib-default", path
...
def collect_render_targets(node, path=None, non_leaf_anc=None, out=None):
    """
    深度优先遍历整棵树，收集每个节点的渲染目标信息。

    返回列表元素结构：
      {
        "path": [根, ..., 当前节点],           # 渲染时用于展开/高亮路径
        "title": 当前节点标题,                 # 输出文件名来源
        "folder_parts": [...],                # 在画布目录下的相对目录片段
        "is_non_leaf": True/False
      }

    树形落盘规则：
      - 非叶子节点：在“以自身命名”的文件夹内保存同名图片
      - 叶子节点：在“父级非叶子节点”对应的文件夹内保存图片

    注意：
      - 这里保留原始标题用于“路径匹配与渲染定位”
      - 文件名清洗在 node_filename() 阶段单独处理，避免影响 --path 匹配行为
    """
    if out is None:
        out = []
    ...
    children = [c for c in get_children(node) if "You are here" not in c.get("title", "")]
    is_non_leaf = bool(children)

    curr_path = (path or []) + [title]
    ...
    for c in children:
        collect_render_targets(c, curr_path, child_non_leaf_anc, out)
...
def draw_node(ax, n):
    ...
    # 优先使用解析后的 FONT_PROP（可通过 FONT_FILE 强制指定字体文件）。
    text_kwargs = {
        "ha": "center", "va": "center",
...
def strip_parenthesized(text):
    """
    文件名清洗函数：删除标题中的括号内容（含括号本身），支持：
      - 中文全角括号：（...）
      - 英文半角括号：(...)

    使用非贪婪匹配，多个括号段分别删除，不跨段吞并中间文本。
    同时删除括号段两侧的空格，避免留下多余间隔。
    另外会删除“最后一个冒号（：或:）及其后内容”，用于去掉副标题。
    该规则只用于“文件名”，不改渲染路径。
    """
...
def node_filename(title):
    """
    文件名仅使用当前节点名，不拼接父路径。
    同时会先删除标题中的括号内容，避免文件名过长。

    规则顺序：
      1) strip_parenthesized() 清洗语义噪声
      2) sanitize_fs_name() 替换非法字符
      3) UTF-8 字节长度截断（220 字节）

    备注：
      - 若清洗后为空，回退到原标题，减少重名“未命名节点”概率
    """
    cleaned = strip_parenthesized(title)
    # 若去括号后为空（例如标题本身只有括号内容），回退到原标题，避免大量“未命名节点”冲突。
    base = sanitize_fs_name(cleaned if cleaned else title)
    while len(base.encode('utf-8')) > 220:
        base = base[:-1]
    return f"{base}.png"


def merge_sheet_and_tree_folders(sheet_name, tree_folders):
    """
    合并“画布目录 + 树形目录”并做首层去重。
    ...
    判断策略：
      - 先用 sanitize_fs_name 统一比较口径
      - 仅去重首层，深层目录保持原样
    """
    parts = [sheet_name, *(tree_folders or [])]
    parts = [p for p in parts if p]
    if len(parts) >= 2 and sanitize_fs_name(parts[0]) == sanitize_fs_name(parts[1]):
        parts.pop(1)
    return parts
...
示例：
  ...
  python xmind_renderer_v5.py -p 哲学/总论      只渲染该路径前缀下的节点
  python xmind_renderer_v5.py -s 哲学 -p 总论    在指定画布内渲染该局部
  python xmind_renderer_v5.py -i 其他.xmind     指定其他 XMind 文件
  python xmind_renderer_v5.py -o my_output      指定输出目录
        """
    )
    ...
    # 启动即解析字体并打印最终命中结果，便于快速定位“字体名可写但实际未命中”的问题。
    FONT_PROP, resolved_font_name, resolved_font_path = resolve_font()
    print(f"🅰️ 使用字体：{resolved_font_name}")
    print(f"   字体文件：{resolved_font_path}")
    ...
    for sheet_title, root in sheets:
        ...
        targets = collect_render_targets(root)
        ...
        if only_path:
            ...
            if not filtered_paths:
                ...
                continue
            keep = {tuple(p) for p in filtered_paths}
            # 路径过滤基于完整 path 精确命中；目录/文件名清洗不参与匹配，避免误筛选。
            targets = [t for t in targets if tuple(t["path"]) in keep]
            print(f"   路径过滤：{'/'.join(matched_prefix)}")

        print(f"   共 {len(targets)} 个节点")
        ...
        for i, target in enumerate(targets):
            ...
            # 子目录：output_dir / 画布名称 / 非叶子层级目录...
            # 若画布名与根节点同名，则自动去掉重复的一层目录。
            merged_folders = merge_sheet_and_tree_folders(sheet_title, target["folder_parts"])
            sub_dir = os.path.join(output_dir, *[sanitize_fs_name(p) for p in merged_folders if p])
            os.makedirs(sub_dir, exist_ok=True)

            fname    = node_filename(target["title"])
            out_path = os.path.join(sub_dir, fname)

            print(f"  [{i+1:3d}/{len(targets)}] {'→'.join(path[-3:])}")
            try:
