...
def compute_sh(title, depth):
    """基于实际渲染盒子高度 + 间距，返回节点占用的 y 空间。"""
    fs = 14 if depth == 0 else (10 if depth == 1 else 9)
    n_lines = len(wrap_text(title).split("\n"))
    bh = max(n_lines * fs * 0.024 + 0.18, 0.34)
    if depth == 0:
        bh = max(bh, 0.50)
    return max(Y_MIN_SH, bh + 0.20)   # +0.20 为节点间留白
...
def build_collapsed_tree(node, depth, bc, side, path_set, target_title, here_label, side_map):
    ...
    t = {
        ...
        "x": x, "y": 0.0, "kids": [],
        "sh": compute_sh(title, depth),   # 用实际盒子高度计算
    }
    ...
    if depth == 0:
        ...
        if is_target:
            t["kids"].append({
                ...
                "y": 0.0, "kids": [],
                "sh": compute_sh(here_label, depth + 1),  # here 节点也用实际高度
            })
...
def _assign_y_side(kids):
    if not kids:
        return 0.0
    cursor = 0.0
    # 反向迭代：第一个子节点分配到最大 y（图片顶部）
    for k in reversed(kids):
        _assign_y_rec(k, cursor)
        cursor += k["sh"]
    return (kids[-1]["y"] + kids[0]["y"]) / 2.0
...
def draw_edges(ax, t):
    ...
    for k in t["kids"]:
        ...
        if side == "right":
            x_start = px + p_bw / 2 + 0.06
            x_end   = cx - k_bw / 2 - 0.06
        else:
            x_start = px - p_bw / 2 - 0.06
            x_end   = cx + k_bw / 2 + 0.06

        # "主干+分支"风格：曲线紧贴父节点侧垂直走，再水平接入子节点
        # trunk_x 在父节点与子节点之间靠近父节点的位置
        trunk_ratio = 0.15
        trunk_x = x_start + (x_end - x_start) * trunk_ratio

        # 四段控制点：起点 → 紧贴 trunk_x 处 → 抵达子节点 y → 终点
        verts = [
            (x_start, py),
            (trunk_x,  py),    # CP1：贴近父节点，水平出发
            (trunk_x,  cy),    # CP2：在 trunk_x 处垂直走到子节点 y
            (x_end,    cy),    # 终点：水平接入子节点
        ]
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="学海计划 XMind 批量渲染")
    parser.add_argument("--sheet", "-s", default="", help="只渲染指定画布（留空则渲染全部），例如：-s 哲学")
    parser.add_argument("--input", "-i", default=INPUT_FILE, help=f"XMind 文件路径（默认：{INPUT_FILE}）")
